...
interface AesingFlowInbound {
    protocol?: string;
    settings?: AesingFlowInboundConfig;
    streamSettings?: {
        network?: string;
        security?: string;
        tlsSettings?: {
...
export class XRayConfig {
    ...
    private validate(): void {
        ...
        for (const inbound of this.config.inbounds) {
            ...
            this.validateAesingFlow(inbound);
        }
    }
    ...
    private validateAesingFlow(inbound: InboundConfig): void {
        const aesingFlowInbound = inbound as unknown as AesingFlowInbound;
        if (aesingFlowInbound.protocol !== 'aesingflow') return;

        const settings = aesingFlowInbound.settings;
        const streamSettings = aesingFlowInbound.streamSettings;

        if (!settings || !Array.isArray(settings.clients)) {
            throw new Error('AesingFlow settings.clients must be an array.');
        }
        if (
            settings.maxStreams !== undefined &&
            (!Number.isInteger(settings.maxStreams) || settings.maxStreams < 1)
        ) {
            throw new Error('AesingFlow settings.maxStreams must be a positive integer.');
        }
        if (
            settings.congestionControl !== undefined &&
            settings.congestionControl !== 'brutal' &&
            settings.congestionControl !== 'cubic'
        ) {
            throw new Error('AesingFlow settings.congestionControl must be "brutal" or "cubic".');
        }
        if (
            settings.brutalBps !== undefined &&
            (!Number.isInteger(settings.brutalBps) || settings.brutalBps < 1)
        ) {
        ...
        for (const certificate of tlsSettings.certificates) {
            if (
                !this.isRemnawaveCertificatePath(certificate.keyFile, '.key') ||
                !this.isRemnawaveCertificatePath(certificate.certificateFile, '.pem')
            ) {
                throw new Error(
                    'AesingFlow certificates must use .key/.pem files under /var/lib/remnawave/configs/xray/ssl/.',
                );
            }
        }
    }

    private isRemnawaveCertificatePath(path: string | undefined, extension: string): boolean {
        const root = '/var/lib/remnawave/configs/xray/ssl/';
        return !!path && path.startsWith(root) && path.endsWith(extension) && !path.includes('..');
    }
